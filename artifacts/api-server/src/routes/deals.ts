...
router.post("/deals", requireAuth, async (req, res): Promise<void> => {
  ...
  const [deal] = await db
    ...
    .values({
      ...
      currency: (user.country as string) === "Tunisia" ? "EUR" : ((user.currency as string | null) || ({ UAE:"AED", KSA:"SAR", KE:"KES", Kenya:"KES", NG:"NGN", Nigeria:"NGN", TN:"TND", Egypt:"EGP", Ethiopia:"ETB", Ghana:"GHS" } as Record<string,string>)[(user.country as string) ?? ""] || "AED"),
      ...
      leadSource: (data as any).leadSource ?? null,
      invoiceSeq,
    })
    .returning();
