...
function fmtCount(n: number) { return n ? String(n) : ""; }
...
function groupByMonth(weeks: WeekRow[]): Array<{ monthKey: string; monthLabel: string; monthYear: number; monthIdx: number; rows: WeekRow[] }> {
  const groups: Array<{ monthKey: string; monthLabel: string; monthYear: number; monthIdx: number; rows: WeekRow[] }> = [];
  for (const w of weeks) {
    ...
    if (last && last.monthKey === key) {
      ...
      const monthYear = parseInt(w.monthYear);
      const monthIdx  = MONTH_NAMES.indexOf(w.monthName) + 1;
      groups.push({ monthKey: key, monthLabel: `${w.monthName} ${w.monthYear}`, monthYear, monthIdx, rows: [w] });
    }
  }
  return groups;
}
...
function handleExportExcel(weeks: WeekRow[], startMonth: string, startYear: string, endMonth: string, endYear: string) {
  ...
  aoa.push([]);
  aoa.push(["Months", "Payment Receipt", "", "", "", "", "Sales in Process", "", "", "", ""]);
  aoa.push(["", "Order Closed", "", "Down Payment Amount", "Total Amount", "", "Quotation Sent", "", "Order Confirmed", "", "Total Amount"]);
  aoa.push(["", "Total Orders", "Amount", "Amount", "Amount", "", "Total Quotations", "Amount", "Total Orders", "Amount", "Amount"]);
  ...
  for (const group of groups) {
    ...
    for (const w of group.rows) {
      aoa.push([
        ...
        fmtCount(w.orderClosedCount) || "", fmtExport(w.orderClosedAmount) || "",
        fmtExport(w.downPayment) || "", fmtExport(w.totalPaymentReceipt) || "", "",
        fmtCount(w.quotationSentCount) || "", fmtExport(w.quotationSentAmount) || "",
        fmtCount(w.orderConfirmedCount) || "", fmtExport(w.orderConfirmedAmount) || "",
  ...
  aoa.push(["TOTAL", fmtCount(totOC), fmtExport(totOCA), fmtExport(totDP), fmtExport(totPR), "",
  ...
  ws["!cols"] = [
    ...
    { wch: 3 }, { wch: 16 }, { wch: 13 }, { wch: 13 }, { wch: 13 }, { wch: 13 },
  ];
...
function DealDrillDownModal({
  drillDown, filterSpId, onClose,
}: { drillDown: DrillDown | null; filterSpId: string; onClose: () => void }) {
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] flex flex-col">
        <DialogHeader className="shrink-0">
          <DialogTitle className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs font-normal text-muted-foreground mt-0.5">{drillDown?.weekLabel}</p>
            </div>
          </DialogTitle>
        </DialogHeader>
        <div className="flex-1 overflow-auto min-h-0">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground text-sm">No deals found for this period and stage.</div>
          ) : (
                ...
                {deals.map((deal, i) => (
                  <tr key={deal.id} className={i % 2 === 0 ? "bg-background" : "bg-muted/20"}>
                    ...
                    <td className="border border-border px-3 py-2 text-xs text-muted-foreground whitespace-nowrap">
                      {deal.dealStartDate
                        ? new Date((typeof deal.dealStartDate === "string" ? deal.dealStartDate.split("T")[0] : deal.dealStartDate) + "T00:00:00")
...
export default function SalesBreakdown() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reports/sales-breakdown?${params}`)
      ...
      .catch(() => setWeeks([]))
      .finally(() => setLoading(false));
  }, [me, startYear, startMonth, endYear, endMonth, filterSpId, selectedRegion]);

  const yearOptions = Array.from({ length: 5 }, (_, i) => String(currentYear - i));
  const groups = groupByMonth(weeks);

  // Build the unique month list for historical rate fetching
  const uniqueMonths = useMemo(() => {
    const seen = new Set<string>();
    const result: Array<{ year: number; month: number }> = [];
    for (const g of groups) {
      const key = `${g.monthYear}-${g.monthIdx}`;
      if (!seen.has(key)) { seen.add(key); result.push({ year: g.monthYear, month: g.monthIdx }); }
    }
    return result;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [groups.map((g) => `${g.monthYear}-${g.monthIdx}`).join(",")]);
  ...
  function openDrill(w: WeekRow, stage: string, columnLabel: string) {
    setDrillDown({
      ...
      weekStart: w.weekStart, weekEnd: w.weekEnd, stage, columnLabel,
    });
  }
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="p-6 max-w-full mx-auto space-y-4 w-full">
        {/* Page header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => handleExportExcel(weeks, smLabel, startYear, emLabel, endYear)} disabled={weeks.length === 0}>
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {groups.length === 0 ? (
                    ...
                    groups.map(({ monthKey, monthLabel, monthYear, monthIdx, rows }) => {
                      ...
                      // Per-month conversion rate
                      const monthRate = getRate(monthYear, monthIdx);
                      const fmtM = (n: number) => isSameCurrency ? fmt(n) : fmtR(n, monthRate);
                        ...
                        <>
                              ...
                              <div className="flex items-center justify-between gap-4">
                                ...
                                {!isSameCurrency && (
                                  <MonthRateCell
                                    ...
                                    isCurrentMonth={rateIsCurrent(monthYear, monthIdx)}
                                    onEdit={(r) => setOverride(monthYear, monthIdx, r)}
                                  />
                                )}
                              </div>
                            </td>
                          ...
                          {rows.map((w, wi) => (
                            <tr key={w.weekStart} className={wi % 2 === 0 ? "" : "bg-muted/5"}>
                              ...
                              <td className={`${tdBase} bg-green-900/10 ${w.orderClosedCount ? clickable : ""}`}
                              ...
                              <td className={`${tdBase} bg-green-900/10 ${w.orderClosedAmount ? clickable : ""}`}
                                ...
                                {fmtM(w.orderClosedAmount)}
                              </td>
                              <td className={`${tdBase} bg-green-900/5 ${w.downPayment ? clickable : ""}`}
                              ...
                              <td className={`${tdBase} bg-green-900/5 font-medium ${w.totalPaymentReceipt ? clickable : ""}`}
                          ...
                          <tr key={`total-${monthKey}`} className="bg-muted/50 border-t border-border font-semibold">
                            <td className={`${tdBase} text-left text-xs font-bold`}>Total {rows[0]?.monthName}</td>
                ...
                {/* Grand Totals footer — uses global live rate */}
                {weeks.length > 0 && (
                  <tfoot>
        ...
        <DealDrillDownModal drillDown={drillDown} filterSpId={filterSpId} onClose={() => setDrillDown(null)} />
      </div>
    </div>
  );
}
